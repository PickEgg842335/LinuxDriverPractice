...
// To do: initialize the device
    vsprintf(Message, "I already told you %d times Hello world\n", counter++);
    msg_Ptr = Message;
...
{
    int bytes_read = 0;
/*    int msglength = sizeof(strlen(msg_Ptr));
    // Actually put the data into the buffer
    for(int i = 0; i < msglength; i++)
    {
	// To do: copy data to the user space buffer.
        put_user(*(msg_Ptr + i), buffer + i);
        bytes_read++;
    }
...
/*    if(length > 0)
    {
        memset(msg_Ptr, 0, sizeof(strlen(msg_Ptr)));
    ...
    printk("I get %d bytes from you, this data is %s\n", bytes_write, msg_Ptr);*/
    return bytes_write;
}
MODULE_LICENSE("GPL");
